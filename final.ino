...
// Distance sensor
#define _DIST_ALPHA 0.28   // [2959] ema 필터에 적용할 알파값

// filtering
static long apt = 0;
#define interval 2 // miliseconds
...
// Servo range
#define _DUTY_MIN 1600 // [2952] 서보의 최소 각도값
#define _DUTY_NEU 1700 // [2952] 서보의 중간 각도값
#define _DUTY_MAX 1850 // [1691] 서보의 최대 각도

// Servo speed control
#define _SERVO_ANGLE 30  //[2967] 서보 각도 설정
#define _SERVO_SPEED 710  //[2959] 서보의 속도 설정

// Event periods
#define _INTERVAL_DIST 20  //[2959] 센서의 거리측정 인터벌값
#define _INTERVAL_SERVO 20 //[2967] 서보 INTERVAL값 설정
#define _INTERVAL_SERIAL 200  //[2959] 시리얼 모니터/플로터의 인터벌값 설정

// PID parameters
#define _KP 1.01 // 비례이득
...
// PID variables
float error_curr, error_prev, control, kp, pterm, dterm, iterm;
...
////////////////////
  ...
  // duty_target = f(duty_neutral, control)
    if(error_curr > 0) {
      duty_target = 1700 + (1.5 * pterm); 
    }else{
      duty_target = 1700 + pterm;
    }
...
float ir_distance_filtered(void){ // return value unit: mm
  ...
  if (apt <= 0) {
    ...
    float filter = lambda / (1 + lambda) * calidist + 1 / (1 + lambda) * prev;
    float prev = filter;
    return filter;
  }
}
